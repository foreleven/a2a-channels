 ...
 * tunnel.  The relay CLI connects over `GET /ws/a2a/{agentId}` using the
 * relayToken for authentication and then handles A2A JSON-RPC frames locally.
 */
export interface WsTunnelAgentConfig {
  readonly transport: "ws-tunnel";
  /**
   * Secret token used to authenticate the relay CLI's WS connection.
   * Auto-generated by the gateway on agent registration; only obtainable once
   * unless regenerated via `POST /api/agents/:id/regenerate-token`.
   */
  readonly relayToken: string;
  /** Executor the relay CLI should use to process inbound messages. */
  readonly executor: WsTunnelExecutorConfig;
