...
- Domain events / outbox 用于唤醒 runtime 收敛，不作为配置真相来源。
- Runtime 从 repository 读取最新 desired state，经过 scheduler / coordinator 变成本节点执行命令。
...
DomainEventBus
  -> DomainEventBridge
  -> LocalRuntimeEventBus.broadcast(BindingChanged / AgentChanged)
  -> LocalScheduler debounce reconcile
...
HTTP command
  ...
  -> outbox / DomainEventBus
  -> DomainEventBridge
  -> RuntimeEventBus.broadcast(BindingChanged / AgentChanged)
  -> LocalScheduler.scheduleReconcile()
  -> RuntimeAssignmentCoordinator.reconcile()
...
### 6.2 Outbox / DomainEventBus 是 fast-path 唤醒

Domain events 的职责是唤醒 runtime 尽快 reconcile。

它们不应该被理解为：

- runtime 的唯一恢复来源；
- 完整 Event Sourcing event log；
- 可以跳过 repository 查询的完整命令载荷。

如果 runtime 错过事件，下一次 scheduler reconcile 仍应从 repository state 收敛。
...
2. Domain events / outbox 只作为 runtime fast-path wakeup，不作为完整事件溯源。
