...
DomainEventBus         best-effort fast-path 通知
Redis（集群模式）        决定当前谁运行某个 Binding
...
当前代码只新增了 Phase 2 的 runtime bootstrap 选择边界：可以在启动时区分 `LocalScheduler` 和 `LeaderScheduler`，并预留 Redis ownership gate 接口；但 `LeaderScheduler` 仍是占位实现，真实的 leader lease、binding lease 和跨实例调度还没有接入。
...
Redis（cluster mode）
  - instance membership
  - binding assignment / ownership leases
...
Agent 和 Channel Binding 是低频配置，state tables 直接就是读模型：

- Admin UI 查询直接读 `agents`、`channel_bindings` 表。
...
Command
  ...
  -> Outbox Worker publishes domain event
  -> DomainEventBus fast-path notification wakes local Scheduler/Lead
  -> local Scheduler/Lead scans or reloads affected binding
  -> local ownership grant
...
ownership gate = local grant
binding distribution = schedule(bindingId) to local instance
```

这样单实例和集群模式的主流程一致，区别只在 ownership gate：

- 单实例：本地 grant，不需要 Redis。
- 集群：必须 acquire Redis binding lease。

`RelayRuntime` 在单实例模式下也不启动时加载所有 bindings；它从空状态启动，由 local Scheduler/Lead 逐个 attach enabled bindings。
...
Command
  ...
  -> Outbox Worker publishes domain event
  -> distributed notification wakes leader / instances
  -> cluster leader schedules binding distribution
  -> selected instance tries to acquire Redis lease
  -> Redis lease enforces single active owner
  -> owning RelayRuntime starts/stops connection
```
